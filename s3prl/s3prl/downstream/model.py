...
class MCDropFrameLevel(nn.Module):
    def __init__(self, input_dim, output_dim, p=0.0, hiddens=None, activation='ReLU', **kwargs):
        super().__init__()
        self.p = p
    ...
    def forward(self, hidden_state, features_len=None, mc: bool = False):
        hidden_state = self.hiddens(hidden_state)
        if mc:
            hidden_state = F.dropout(hidden_state, p=self.p, training=True)   # functional dropout
        else:
...
class UtteranceLevel(nn.Module):
    ...
    def forward(self, hidden_state, features_len=None, mc=False):
        ...
        # if self.post_net.p:
        if hasattr(self.post_net, "p"):
            logit, features_len = self.post_net(pooled, features_len, mc=mc)
        else:
            logit, features_len = self.post_net(pooled, features_len)

        return logit, pooled, features_len
...
class MeanPooling(nn.Module):
    ...
    def forward(self, feature_BxTxH, features_len, **kwargs):
        ...
        # print(f"after pooled: {torch.stack(agg_vec_list).shape}")
