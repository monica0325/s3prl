...
class DataManager:
    ...
    """
    def __init__(self, env_path, corpus):
    ...
    def get_utt_list(self, split_type, lbl_loc):
        ...
        with open(label_path, 'r') as f:
            ...
            for row in csv_reader:
                utt_id = row[0]
                if self.corpus == "CREMA-D":
                    stype = row[3]
                elif self.corpus == "PODCAST" or self.corpus == "IEMOCAP":
                    stype = row[6]
                else:    
                    stype = row[-1] #[6] #-1
                if stype == sid:
...
def prepare_datasets(datarc, config_path):
    ...
    """
    dam = DataManager(config_path, datarc['corpus'])

    audio_path = os.path.join(datarc['root'], datarc['corpus'], "Audios")
    
    if datarc['corpus'] == 'CREMA-D' or datarc['corpus'] == 'PODCAST' or datarc['corpus'] == 'IEMOCAP':
