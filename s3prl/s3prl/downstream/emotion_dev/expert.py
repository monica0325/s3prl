...
class DownstreamExpert(nn.Module):
    ...
    def __init__(self, upstream_dim, runner, downstream_expert, expdir, **kwargs):
        ...
        self.labels_path = self.datarc['root'] + self.datarc['corpus'] + '/' + self.datarc['p_or_s'] + '/'+ self.datarc['src'] + "/labels_consensus_" + self.datarc['test_fold'].replace("fold","") + ".csv" #"_smooth.csv"
        self.config_path = self.datarc['root'] + self.datarc['corpus'] + '/' + self.datarc['p_or_s'] + '/'+ self.datarc['src'] + "/config.json"
        ...
        print(f"train_labs: {train_labs.shape}")
    ...
    def forward(self, mode, features, labels, filenames, records, **kwargs):
        ...
        report_dict = classification_report(labels_binary.cpu().numpy(force=True), predictions_binary.cpu().numpy(force=True),target_names=all_emotions,output_dict=True)
        records['acc'] += [accuracy_score(labels_np, predicted_np)]
        records['precision'] += [report_dict['macro avg']['precision']]
        records['recall'] += [report_dict['macro avg']['recall']]
        records['macro-f1'] += [report_dict['macro avg']['f1-score']]
    ...
    def log_records(self, mode, records, logger, global_step, **kwargs):
        save_names = []
        for key in ["acc", "loss", "precision", "recall", "macro-f1"]:
            values = records[key]
            ...
            with open(Path(self.expdir) / "log.log", 'a') as f:
                ...
                elif key == 'acc' or key == 'macro-f1' or key == 'precision' or key == "recall":
                    print(f"{mode} {key}: {average}")
                    f.write(f'{mode} {key} at step {global_step}: {average}\n')
