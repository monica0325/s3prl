...
def get_downstream_args():
    ...
    # options
    parser.add_argument('--filter_thre', type=float, default=0.75)
    parser.add_argument('--multi_generation', action='store_true', help='whether to filter whole dataset by mutual information')
    parser.add_argument('--ent_mean_based_sample_selection', action='store_true', help='whether to filter whole dataset by entropy mean')
    parser.add_argument('--ent_each_based_sample_selection', action='store_true', help='whether to filter whole dataset by entropy each')
    parser.add_argument('--mi_based_sample_selection', action='store_true', help='whether to filter whole dataset by mutual information')
    parser.add_argument('--fu_thre', default="KL", choices=["KL", "N/A"], help='Similarity threshold types')
    parser.add_argument('--fusion_enabling', action='store_true', help='whether to derive pseudo labels by distribution fusion')
    parser.add_argument('--fusion_equal', action='store_true', help='whether to fuse probs equally')
    parser.add_argument('--fusion_ent', action='store_true', help='whether to fuse probs by entropy weighting')
    parser.add_argument('--cos_tau', type=float, default=0.9)
    parser.add_argument('--kl_tau', type=float, default=0.06)
    parser.add_argument('--ema_decay', type=float, default=0.999)
    parser.add_argument('--mc_sampling', action='store_true', help='whether to use monte-carlo dropout to derive confidence')
    parser.add_argument('--use_acr', action='store_true', help='whether to use pace function type weight based on kl divergence of two probs')
    parser.add_argument('--use_curriculum', action='store_true', help='whether to use curriculum sample selection based on difficulty')
    parser.add_argument('--lambda_beta', type=float, default=1.0)
    parser.add_argument('--class_weights', action='store_true', help='whether to weight class-wise loss')
    parser.add_argument('--topk', default=32, type=int)
    parser.add_argument('--fbnm', action='store_true', help='whether to add fbnm loss')
    parser.add_argument('--lambda_distill', type=float, default=1.0)
    parser.add_argument('--lambda_neark', type=float, default=1.0)
    parser.add_argument('--lambda_nearkk', type=float, default=1.0)
    parser.add_argument('--interval_iter', type=int, default=30)
    parser.add_argument('--lambda_m', type=float, default=0.5)
    parser.add_argument('--lambda_ad', type=float, default=0.2)
    ...
    parser.add_argument('--momentum', type=float, default=0.2, help="update momentum for the memory bank")
    parser.add_argument('--source_ckpt', type=str, default='', help='Path to source‑domain checkpoint for Featurizer / Downstream')
    parser.add_argument('--downstream_trainable', action='store_true', help='Fine-tune, set downstream.train(). Default is downstream.eval()')
    parser.add_argument('--featurizer_trainable', action='store_true', help='Fine-tune, set featurizer.train(). Default is featurizer.eval()')
    parser.add_argument('--lambda_im', default=1.0, type=float)
    parser.add_argument('--lambda_ent', default=1.0, type=float)
    parser.add_argument('--lambda_fbnm', default=1.0, type=float)
    parser.add_argument('--lambda_div', default=1.0, type=float)
    parser.add_argument('--lambda_pl', default=1.0, type=float)
    parser.add_argument('--K', default=5, type=int)
    parser.add_argument('--KK', default=5, type=int)
    parser.add_argument('--bank_store', action='store_true', help='implement feature/score bank')
    ...
    if args.expdir is None:
        args.expdir = f'result/downstream/{args.downstream}/{args.expname}'
